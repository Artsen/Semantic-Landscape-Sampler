...
from app.services.projection import build_feature_matrix, cluster_with_fallback, compute_umap
...
class RunService:
    ...
    async def update_run(self, session, *, run_id: UUID, payload: RunUpdateRequest) -> Run:
    ...
    async def sample_run(
        ...
        *,
        ...
        if options.include_segments:
            segment_drafts = flatten_drafts(
                make_segment_drafts(
                    ...
                    word_window=_SETTINGS.segment_word_window,
                    word_overlap=_SETTINGS.segment_word_overlap,
                )
                for response in responses
            )
        ...
        response_embeddings = await self._embed_responses(responses)
        segment_embeddings: EmbeddingBatch | None = None

        if segment_drafts:
            segment_embeddings = await self._openai.embed_texts([seg.text for seg in segment_drafts])
            for seg, vector in zip(segment_drafts, segment_embeddings.vectors, strict=False):
            ...
            segment_projection = compute_umap(segment_feature_matrix, min_dist=_SETTINGS.projection_min_dist)
        ...
        projection_result = compute_umap(feature_matrix, min_dist=_SETTINGS.projection_min_dist)
    ...
    async def _embed_responses(self, responses: Sequence[Response]):
    ...
    def _build_segment_edges(self, run_id: UUID, segments: Sequence[SegmentDraft]) -> list[SegmentEdge]:
        vectors = [np.asarray(seg.embedding, dtype=float) for seg in segments if seg.embedding]
...
async def load_run_with_details(session, run_id: UUID):
    ...
    results = await session.exec(select(Response).where(Response.run_id == run_id).order_by(Response.index))
