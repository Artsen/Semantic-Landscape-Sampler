...
export const RunMetadataBar = memo(function RunMetadataBar() {
    const { results, runMetrics, projectionMethod, projectionVariants, projectionWarnings, isProjectionLoading } = useRunStore((state) => ({
    ...
    const badges = useMemo(() => {
        ...
        if (projectionMetadata) {
            projectionTitle.push(`Source: ${projectionMetadata.fromCache ? "cached" : "fresh"} - ${projectionMetadata.pointCount}/${projectionMetadata.totalCount} points`);
    ...
    const renderGauge = (label, value) => {
        if (value == null || Number.isNaN(value)) {
            return (_jsxs("div", { className: "flex items-center gap-2 text-[11px] text-slate-500", children: [_jsx("span", { children: label }), _jsx("span", { children: "--" })] }));
        }
        const clamped = Math.max(0, Math.min(1, value));
        const percent = clamped * 100;
        return (_jsxs("div", { className: "flex items-center gap-2 text-[11px] text-slate-400", title: gaugeHints[label] ?? "Projection quality indicator", children: [_jsx("span", { children: label }), _jsx("div", { className: "h-1.5 w-20 rounded-full bg-slate-700/60", children: _jsx("div", { className: "h-full rounded-full bg-cyan-400", style: { width: `${percent}%` } }) }), _jsx("span", { className: "text-slate-300", children: percent.toFixed(1) + "%" })] }));
    };
    return (_jsxs("div", { className: "glass-panel flex items-center justify-between rounded-2xl border border-slate-800/60 bg-slate-950/60 px-4 py-3 text-xs text-slate-300", children: [_jsxs("div", { className: "flex flex-col gap-2", title: "Quick facts about the current run and its embedding quality.", children: [_jsx("p", { className: "text-[11px] text-slate-500", children: "Hover any badge to learn how it influences the landscape." }), _jsx("div", { className: "flex flex-wrap gap-2", children: badges.map((badge) => (_jsx("span", { className: badgeClass, title: badge.title ?? badgeHints[badge.label] ?? "Metadata value", children: badge.label + ": " + badge.value }, badge.label))) }), gauges.length ? (_jsx("div", { className: "flex flex-wrap gap-3", children: gauges.map((item) => renderGauge(item.label, item.value)) })) : null] }), _jsx("span", { className: "text-[11px] uppercase tracking-wide text-slate-500", children: "Saved " + dateFormatter.format(new Date(run.created_at)) })] }));
});
